...
VALUE_AWARE_STRATEGIES = ("value_rank",)  # same "only the REAL simulate_*_signals() accepts
                                                                   # rank_column" precedent as
                                                                   # QUALITY_AWARE_STRATEGIES -- a
                                                                   # SEPARATE tuple because value_rank's
                                                                   # rank_column ALSO needs a fresh SEC
                                                                   # EDGAR fetch (book value per share,
                                                                   # not ROE), combined with a Close-
                                                                   # price panel (already-fetched OHLCV,
...
CASH_PROFITABILITY_AWARE_STRATEGIES = ("cash_profitability_rank",)  # same "only the REAL
                                                                   # simulate_*_signals() accepts
                                                                   # rank_column" precedent as
                                                                   # ACCRUALS_AWARE_STRATEGIES -- a
                                                                   # SEPARATE tuple, own SEC EDGAR
                                                                   # fetch (CFO/Assets), see
                                                                   # sec_fundamentals.build_cash_profitability_panel()
...
STRENGTH_CAP_FIELD = {
    ...
    "cash_profitability_rank": "cash_profitability_strength_cap_pct",
}
...
def main():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "value_rank", "analyst_revision", "accruals_rank", "cash_profitability_rank",
        ],
        default="rsi",
    ...
    if args.strategy in VALUE_AWARE_STRATEGIES:
        # Same "genuinely fetches new data" cost as quality_rank_frame above
        # -- a SEPARATE SEC EDGAR concept (book value per share, not ROE),
        # combined with a Close-price panel (no new fetch, OHLCV already in
        # hand) to form the Price-to-Book ratio this strategy ranks on.
        full_date_range = pd.date_range(start, end, freq="D")
        print(f"\nFetching point-in-time Book Value Per Share for {len(ticker_data)} ticker(s) from "
              "SEC EDGAR (one request per ticker, paced -- this is the slow part)...")
        bvps_panel = sec_fundamentals.build_book_value_per_share_panel(list(ticker_data.keys()), full_date_range)
        covered = bvps_panel.notna().any().sum()
        print(f"Got real SEC EDGAR fundamentals for {covered}/{len(ticker_data)} ticker(s) "
              "(tickers with none -- e.g. Canadian cross-listed names -- read as 'never fires').")
    ...
    if args.strategy in CASH_PROFITABILITY_AWARE_STRATEGIES:
        full_date_range = pd.date_range(start, end, freq="D")
        print(f"\nFetching point-in-time Cash Profitability for {len(ticker_data)} ticker(s) from "
              "SEC EDGAR (one request per ticker, paced -- this is the slow part)...")
        cash_profitability_panel = sec_fundamentals.build_cash_profitability_panel(
            list(ticker_data.keys()), full_date_range,
        )
        covered = cash_profitability_panel.notna().any().sum()
        print(f"Got real SEC EDGAR fundamentals for {covered}/{len(ticker_data)} ticker(s) "
              "(tickers with none -- e.g. Canadian cross-listed names -- read as 'never fires').")
        cash_profitability_rank_frame = swingtrade.compute_cash_profitability_rank_frame(cash_profitability_panel)
    ...
    elif args.strategy == "cash_profitability_rank":
        real_fn, random_fn = swingtrade.simulate_cash_profitability_signals, swingtrade.simulate_random_cash_profitability_entries
        real_label = "Cash_Profitability_Rank-timed"
    else:
    ...
    def value_kwargs(ticker):
        if args.strategy not in VALUE_AWARE_STRATEGIES:
            return {}
    ...
    def cash_profitability_kwargs(ticker):
        if args.strategy not in CASH_PROFITABILITY_AWARE_STRATEGIES:
            return {}
        if cash_profitability_rank_frame is None or ticker not in cash_profitability_rank_frame.columns:
            return {"rank_column": None}
        return {"rank_column": cash_profitability_rank_frame[ticker]}
    for i, (ticker, ohlcv) in enumerate(ticker_data.items()):
        ...
        real = real_fn(
            ...
            **analyst_revision_kwargs(ticker), **accruals_kwargs(ticker), **cash_profitability_kwargs(ticker),
        )
