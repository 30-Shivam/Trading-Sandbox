...
def add_momentum_trade_score(df: pd.DataFrame, config: TradingConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    """Cross-sectional MOMENTUM RANK counterpart to add_trade_score()/
    add_pairs_trade_score()/add_squeeze_breakout_trade_score()/etc. --
    blends RRR and Signal_Strength_Pct (percentile points past
    momentum_top_percentile_min, see momentum_levels_from_frame()) into a
    0-100 Trade_Score for rows produced by momentum_levels_from_frame(),
    reusing signal_for_score()'s thresholds so a momentum Trade_Score means
    the same thing on the same 0-100 scale as every other strategy.

    Distance_to_Buy_Pct is always 0 for this strategy (Buy_Price IS
    Last_Close) -- Signal_Strength_Pct (in percentile points here, not a %
    of price) fills that role instead, same "differentiating term" pattern
    squeeze_breakout/pairs use.

    Hard gate, not just a scoring input: a ticker whose Momentum_Signal is
    False (percentile didn't clear momentum_top_percentile_min, no
    percentile data at all, or outside the shared macro-uptrend/liquidity
    gates) gets Trade_Score=0/Ignore, full stop -- same "not eligible at
    all" semantics as every other strategy's hard gate, not merely a low
    score.
    ...
    strength_clipped = df["Signal_Strength_Pct"].clip(lower=0, upper=config.momentum_strength_cap_pct)
    strength_score = (strength_clipped / config.momentum_strength_cap_pct) * config.distance_score_weight
...
def add_lowvol_trade_score(df: pd.DataFrame, config: TradingConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    """Cross-sectional LOW-VOLATILITY RANK counterpart to add_momentum_trade_score()/
    add_trade_score()/etc. -- blends RRR and Signal_Strength_Pct (percentile
    points past lowvol_top_percentile_min, see lowvol_levels_from_frame())
    into a 0-100 Trade_Score for rows produced by lowvol_levels_from_frame(),
    reusing signal_for_score()'s thresholds so a low-vol Trade_Score means
    the same thing on the same 0-100 scale as every other strategy. Exact
    structural mirror of add_momentum_trade_score().

    Hard gate, not just a scoring input: a ticker whose LowVol_Signal is
    False gets Trade_Score=0/Ignore, full stop -- same "not eligible at all"
    semantics as every other strategy's hard gate."""
    df = df.copy()

    rrr_score = (df["RRR"].clip(lower=0, upper=config.rrr_score_cap) / config.rrr_score_cap) * config.rrr_score_weight

    strength_clipped = df["Signal_Strength_Pct"].clip(lower=0, upper=config.lowvol_strength_cap_pct)
    strength_score = (strength_clipped / config.lowvol_strength_cap_pct) * config.distance_score_weight
    ...
    df["Trade_Score"] = raw_score.where(df["LowVol_Signal"], 0.0).round(1)
    df["Signal"] = df["Trade_Score"].apply(lambda score: signal_for_score(score, config))
    return df
...
def add_quality_trade_score(df: pd.DataFrame, config: TradingConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    """Cross-sectional QUALITY/PROFITABILITY RANK counterpart to
    add_momentum_trade_score()/add_lowvol_trade_score()/etc. -- blends RRR
    and Signal_Strength_Pct (percentile points past
    quality_top_percentile_min, see quality_levels_from_frame()) into a
    0-100 Trade_Score for rows produced by quality_levels_from_frame().
    Exact structural mirror of add_momentum_trade_score().

    Hard gate, not just a scoring input: a ticker whose Quality_Signal is
    ...
    strength_clipped = df["Signal_Strength_Pct"].clip(lower=0, upper=config.quality_strength_cap_pct)
    strength_score = (strength_clipped / config.quality_strength_cap_pct) * config.distance_score_weight
    ...
    df["Trade_Score"] = raw_score.where(df["Quality_Signal"], 0.0).round(1)
    df["Signal"] = df["Trade_Score"].apply(lambda score: signal_for_score(score, config))
    return df


def add_value_trade_score(df: pd.DataFrame, config: TradingConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    """Cross-sectional VALUE RANK counterpart to add_quality_trade_score()
    -- blends RRR and Signal_Strength_Pct (percentile points past
    value_top_percentile_min, see value_levels_from_frame()) into a 0-100
    Trade_Score for rows produced by value_levels_from_frame(). Exact
    structural mirror of add_quality_trade_score().

    Hard gate, not just a scoring input: a ticker whose Value_Signal is
    False gets Trade_Score=0/Ignore, full stop -- same "not eligible at
    all" semantics as every other strategy's hard gate."""
    df = df.copy()
    ...
    strength_clipped = df["Signal_Strength_Pct"].clip(lower=0, upper=config.value_strength_cap_pct)
    strength_score = (strength_clipped / config.value_strength_cap_pct) * config.distance_score_weight
    ...
    df["Trade_Score"] = raw_score.where(df["Value_Signal"], 0.0).round(1)
    df["Signal"] = df["Trade_Score"].apply(lambda score: signal_for_score(score, config))
    return df
...
def add_cash_profitability_trade_score(df: pd.DataFrame, config: TradingConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    """Cross-sectional CASH PROFITABILITY RANK counterpart to
    add_quality_trade_score()/add_accruals_trade_score() -- blends RRR and
    Signal_Strength_Pct (percentile points past
    cash_profitability_top_percentile_min) into a 0-100 Trade_Score for
    rows produced by cash_profitability_levels_from_frame(). Exact
    structural mirror of add_quality_trade_score().

    Hard gate, not just a scoring input: a ticker whose
    Cash_Profitability_Signal is False gets Trade_Score=0/Ignore, full
    stop -- same "not eligible at all" semantics as every other
    strategy's hard gate."""
    df = df.copy()

    rrr_score = (df["RRR"].clip(lower=0, upper=config.rrr_score_cap) / config.rrr_score_cap) * config.rrr_score_weight

    strength_clipped = df["Signal_Strength_Pct"].clip(lower=0, upper=config.cash_profitability_strength_cap_pct)
    strength_score = (strength_clipped / config.cash_profitability_strength_cap_pct) * config.distance_score_weight
    ...
    df["Trade_Score"] = raw_score.where(df["Cash_Profitability_Signal"], 0.0).round(1)
    df["Signal"] = df["Trade_Score"].apply(lambda score: signal_for_score(score, config))
    return df
...
def add_insider_buying_trade_score(df: pd.DataFrame, config: TradingConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    ...
    (distinct insiders beyond insider_min_distinct_buyers, see
    insider_buying_levels_from_frame()) into a 0-100 Trade_Score for rows
    produced by insider_buying_levels_from_frame(), reusing
    signal_for_score()'s thresholds so an insider-buying Trade_Score means
    the same thing on the same 0-100 scale as every other strategy.

    Distance_to_Buy_Pct is always 0 for this strategy (Buy_Price IS
    Last_Close) -- Signal_Strength_Pct fills that role instead, same
    "differentiating term" pattern squeeze_breakout/pairs use.

    Hard gate, not just a scoring input: a ticker whose Insider_Buy_Signal
    is False (no qualifying purchase cluster, or outside the shared
    macro-uptrend/liquidity gates) gets Trade_Score=0/Ignore, full stop --
    same "not eligible at all" semantics as every other strategy's hard
    gate, not merely a low score.
    ...
    strength_clipped = df["Signal_Strength_Pct"].clip(lower=0, upper=config.insider_strength_cap_buyers)
    strength_score = (strength_clipped / config.insider_strength_cap_buyers) * config.distance_score_weight
    ...
    df["Trade_Score"] = raw_score.where(df["Insider_Buy_Signal"], 0.0).round(1)
    df["Signal"] = df["Trade_Score"].apply(lambda score: signal_for_score(score, config))
    return df
